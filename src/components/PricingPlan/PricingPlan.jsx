import React from "react";
import PriceCard from "./PriceCard";

const PricingPlan = () => {
  return (
    <div className="flex gap-6 justify-center flex-col bg-white py-8">
      <div className="w-full flex gap-2 items-center content-center  flex-col font-bold">
        <h1 className="text-xl">
          ABOUT <span className="text-green-700">APPNEW</span>
        </h1>
        <img
          src="data:image/png;base64,"
          alt=""
        />
        <p className="text-center font-normal w-2/3 text-[10px]">
          There are many variations of passages of Lorem Ipsum available, but
          the majorityhave suffered alteration in some form, by injected humour,
          available
        </p>
      </div>
      <div className="flex items-end justify-center gap-3 px-4">
      <PriceCard style1={'py-2'}/>
      <PriceCard style={'mx-0'} style1={'py-2'}/>
      <PriceCard style1={'py-2'}/>
      </div>
    </div>
  );
};

export default PricingPlan;
